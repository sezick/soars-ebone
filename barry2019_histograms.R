...
# Histogram Plot of All Simulated Errors
par(mfrow=c(2,1))
# hist(simvalues , breaks=40 , col=rgb(0.2,0.8,0.5,0.5) , border=F , main="" , xlab="value of the variable", xlim=c(-10,20))
hist(simvalues , breaks=20, main="" , xlab="precipitation (mm)") # takes a couple of seconds
# For bos plot, there are too many data points, so instead we'll draw a random sample
simRandomValues=sample(as.vector(simvalues),100000)
boxplot(simRandomValues,horizontal=T,xlab="Precipitation (mm") # this line takes a while (minutes) and it's not plotting correctly - too many data points?
# Save figure
dev.copy(png,paste(workpath,'histogram+boxplot_ALLerrors.png'))
dev.off()

#hist(simvalues, breaks=seq(0,100,5), main = "Distribution of Simulated Errors")
#hist(simvalues,freq=FALSE, main = "Distribution of Simulated Errors")
#title(xlab = "Maximum Error Field Values", line = 2.2, cex.lab = 1.2)
#title(ylab = "Number of Occurrences", line = 2.5, cex.lab = 1.2)

# max error for each of the individual 1000 error fields
#maxErrorSims = rep(NA,1000)
# for (i in 1:1000) {
#   maxErrorSims[i] = max(simvalues[,i])
# }
# maxErrorsSims[,1:dim(simvalues[2]] = max(simvalues[,1:dim(simvalues)[2]])
#maximum rainfall 
# max error for all of the 1000 scenarios in each pixel
# maxErrorsPixels = rep(NA, length(coords[,1]))
# j=1
# for (j in 1:length(maxErrorsPixels)) {
#   maxErrorsPixels[j] = max(simvalues[j,])
# }
